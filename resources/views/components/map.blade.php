...
@endphp
<div @class(['filament-map h-full']) @if ($pollingInterval) wire:poll.{{ $pollingInterval }} @endif>
    <div {{ $attributes->class(['h-full w-full overflow-hidden', 'rounded-xl' => $rounded]) }} {{ $extraAttributeBag }}>
        <div class="h-full" wire:ignore x-data="{
            ...
            init: function() {
                ...
                @foreach ((array) $tileLayerUrl as $mode => $url)
                    this.tileLayers['{{ $mode }}'] = leaflet.tileLayer('{{ $url }}', {{ json_encode($tileLayerOptions[$mode] ?? $tileLayerOptions) }}); @endforeach
            ...
            @if (count($actions))
                <div {{ $attributes->class(['filament-map-actions']) }}>
                    @foreach ($actions as $action)
                        <div class="filament-map-button justify-center content-center"
                            id="{{ $action->getMapActionId() }}">
                            {{ $action }}
                        </div>
                    @endforeach
                </div>
            @endif
            <div x-ref="map" class="filament-map-container flex-1 relative"
                style="width: 100%; height: {{ $height }}; min-height: 100%; z-index: 0"></div>
        </div>
    </div>
